...
def taglio_raster_corrente_su_area_mean_partial(file_current, ulY, lrY, ulX, lrX, climate_file):

    climate_file = gdal.Open(climate_file)
    x_size_clim = climate_file.RasterXSize
    y_size_clim = climate_file.RasterYSize
    ...
    data_current = gdalnumeric.BandReadAsArray(banda_current)
    print "Il tipo della banda letta con gdalnumeric e' %s" % type(data_current)
    clip_data_current = data_current[ulY:lrY,ulX:lrX]
    np.savetxt('test.out', clip_data_current, delimiter=',')
